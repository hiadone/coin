...
class Choose_model extends CB_Model
{

    /**
     * 테이블명
     */
    public $_table = 'choose';
    ...
    {
        $select = 'choose.*, member.mem_id, member.mem_userid, member.mem_nickname, member.mem_is_admin, member.mem_icon';
        $join[] = array('table' => 'member', 'on' => 'choose.mem_id = member.mem_id', 'type' => 'left');
    ...
    {
        $select = 'choose.*, post.mem_id as post_mem_id, post.post_userid, post.post_username, post.post_nickname, post.post_id,
            post.brd_id, post.post_datetime, post.post_hit, post.post_secret, post.post_title, post.post_choose, post.post_dislike, post.post_image, post.post_del';
        $join[] = array('table' => 'post', 'on' => 'choose.target_id = post.post_id', 'type' => 'inner');
    ...
    {
        $select = 'choose.*, comment.cmt_id, comment.mem_id as cmt_mem_id, comment.cmt_userid, comment.cmt_username,
            comment.cmt_nickname, comment.post_id, comment.cmt_datetime, comment.cmt_secret, comment.cmt_content,
            comment.cmt_content, comment.cmt_choose, comment.cmt_dislike, comment.cmt_del';
        $join[] = array('table' => 'comment', 'on' => 'choose.target_id = comment.cmt_id', 'type' => 'inner');
