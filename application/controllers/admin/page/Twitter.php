 ...
 */
class Twitter extends CB_Controller
{
     ...
     * 페이지 이동시 필요한 정보입니다
     */
    public $pagedir = 'page/twitter';

    /**
     * 모델을 로딩합니다
     */
    protected $models = array('Twitter', 'Twitter_group');

    /**
     * 이 컨트롤러의 메인 모델 이름입니다
     */
    protected $modelname = 'Twitter_model';
    ...
    {
        // 이벤트 라이브러리를 로딩합니다
        $eventname = 'event_admin_page_twitter_index';
    ...
    {
        // 이벤트 라이브러리를 로딩합니다
        $eventname = 'event_admin_page_twitter_write';
        $this->load->event($eventname);
        ...
        $group = $this->Twitter_group_model->get_all_group();
        ...
        if ($form_validation) {
            ...
            if (isset($_FILES) && isset($_FILES['ban_image']) && isset($_FILES['ban_image']['name']) && $_FILES['ban_image']['name']) {
                $upload_path = config_item('uploads_dir') . '/twitter/';
        ...
        if ($form_validation === false OR $file_error !== '') {
            ...
            if (element('ban_image', $getdata) && ($this->input->post('ban_image_del') OR $updatephoto)) {
                // 기존 파일 삭제
                @unlink(config_item('uploads_dir') . '/twitter/' . element('ban_image', $getdata));
            }
            ...
            if ($this->input->post($primary_key)) {
                $this->cache->delete('twitter/twitter-' . element('bng_name', $getdata) . '-random-' . cdate('Y-m-d'));
                $this->cache->delete('twitter/twitter-' . element('bng_name', $getdata) . '-order-' . cdate('Y-m-d'));
    ...
    public function group()
    {
        ...
        // 이벤트 라이브러리를 로딩합니다
        $eventname = 'event_admin_page_twitter_group';
        $this->load->event($eventname);
        ...
        $config = array(
            array(
                'field' => 'bng_name',
                'label' => '위치명',
                'rules' => 'trim|required|alpha_dash|is_unique[twitter_group.bng_name]',
            ),
        );
        $this->form_validation->set_rules($config);
        ...
        if ($this->form_validation->run() === false) {
            ...
            $this->Twitter_group_model->insert($insertdata);
        }
        ...
        $result = $this->Twitter_group_model
        ...
        if (element('list', $result)) {
            foreach (element('list', $result) as $key => $val) {
                ...
                $result['list'][$key]['twitter_count'] = $this->Twitter_model->count_by($countwhere);
            }
        }

        $view['view']['data'] = $result;

        $primary_key = $this->Twitter_group_model->primary_key;
    ...
    {
        // 이벤트 라이브러리를 로딩합니다
        $eventname = 'event_admin_page_twitter_listdelete';
        $this->load->event($eventname);
    ...
    {
        // 이벤트 라이브러리를 로딩합니다
        $eventname = 'event_admin_page_twitter_groupdelete';
        $this->load->event($eventname);
        ...
        if ($this->input->post('chk') && is_array($this->input->post('chk'))) {
            foreach ($this->input->post('chk') as $val) {
                if ($val) {
                    $this->Twitter_group_model->delete($val);
                }
            }
        }
