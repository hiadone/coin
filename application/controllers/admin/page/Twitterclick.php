 ...
 */
class Twitterclick extends CB_Controller
{
     ...
     * 페이지 이동시 필요한 정보입니다
     */
    public $pagedir = 'page/twitterclick';

    /**
     * 모델을 로딩합니다
     */
    protected $models = array('Twitter_click_log');

    /**
     * 이 컨트롤러의 메인 모델 이름입니다
     */
    protected $modelname = 'Twitter_click_log_model';
    ...
    {
        // 이벤트 라이브러리를 로딩합니다
        $eventname = 'event_admin_page_twitterclick_index';
         ...
         */
        $this->{$this->modelname}->allow_search_field = array('bcl_id', 'ban_id', 'bng_name', 'twitter.mem_id', 'twitter_click_log.mem_id', 'bcl_datetime', 'bcl_ip'); // 검색이 가능한 필드
        $this->{$this->modelname}->search_field_equal = array('bcl_id', 'ban_id', 'twitter.mem_id', 'twitter_click_log.mem_id'); // 검색중 like 가 아닌 = 검색을 하는 필드
        $this->{$this->modelname}->allow_order_field = array('bcl_id'); // 정렬이 가능한 필드
        ...
        if (element('list', $result)) {
            foreach (element('list', $result) as $key => $val) {
                ...
                if (element('ban_image', $val)) {
                    $result['list'][$key]['thumb_url'] = thumb_url('twitter', element('ban_image', $val), '80');
                }
                if (element('bcl_useragent', $val)) {
                    $userAgent = get_useragent_info(element('bcl_useragent', $val));
         ...
         */
        $search_option = array('bcl_datetime' => '날짜', 'bcl_ip' => 'IP');
    ...
    {
        // 이벤트 라이브러리를 로딩합니다
        $eventname = 'event_admin_page_twitterclick_graph';
        $this->load->event($eventname);
        ...
        $result = $this->{$this->modelname}->get_twitter_click_count($datetype, $start_date, $end_date, $orderby);
        $sum_count = 0;
        $arr = array();
        $max = 0;
    ...
    {
        // 이벤트 라이브러리를 로딩합니다
        $eventname = 'event_admin_page_twitterclick_listdelete';
        $this->load->event($eventname);
    ...
    {
        // 이벤트 라이브러리를 로딩합니다
        $eventname = 'event_admin_page_twitterclick_cleanlog';
        $this->load->event($eventname);
        ...
        if ($this->form_validation->run() === false) {
            ...
            if ($this->input->post('criterion') && $this->input->post('day')) {
                $deletewhere = array(
                    'bcl_datetime <=' => $this->input->post('criterion'),
                );
                $this->Twitter_click_log_model->delete_where($deletewhere);
                ...
                $countwhere = array(
                    'bcl_datetime <=' => $criterion,
                );
                $log_count = $this->Twitter_click_log_model->count_by($countwhere);
